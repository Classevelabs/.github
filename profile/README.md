...
**Independent software company.** On-device voice-to-text, live speech translation, and open-source engines for AI agents. Research that becomes working software.
...
**Product source is not public.** REX, Lven Instant, Lven Cloud, earslate, and Folio PDF are closed-source; their repositories are private and there is no public mirror. The three projects above are the only ClassEve code with public source. A missing public repository for a ClassEve product is expected, not a sign you are on the wrong account.